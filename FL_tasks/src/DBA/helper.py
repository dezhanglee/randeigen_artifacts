...
class Helper:
    ...
    def randeigen(self, target_model, updates, eps=1./12, sigma=1, itv=ITV):
        """
        ...
        for layer_name in chosen.keys():
            ...
            res = []
            for size_ in sizes:
                temp_res = self.randomized_agg_forced(samples_flatten[:, idx:idx+size_])
                res.append(temp_res)
                idx += size_
            res = torch.cat(res)
    ...
    def power_iteration(self, mat, iterations, device):
        dim = mat.shape[0]
    ...
    def randomized_agg_forced(self, data, eps_poison=0.2, eps_jl=0.1, eps_pow = 0.1, device = 'cuda:0', seed=12):
        n = int(data.shape[0])
        feature_shape = data[0].shape
        n_dim = int(np.prod(np.array(feature_shape)))
        res =  self._randomized_agg(data, eps_poison, eps_jl, eps_pow, 1, 10**-5, device, forced=True, seed=seed) # set threshold for convergence as 1*10**-5 (i.e. float point error)
        return res
    
    
 
    def _randomized_agg(self, data, eps_poison=0.2, eps_jl=0.1, eps_pow = 0.1, threshold = 20, clean_eigen = 10**-5, device = 'cuda:0', forced=False, seed=None):
        if seed:
        ...
        for _ in range(max(int(eps_poison*n), 10)):
            ...
            eigenvalue, eigenvector = self.power_iteration(Y_sq, power_iter_rounds, device)
    ...
    def filterl2_(self, samples, eps=0.2, sigma=1, expansion=20):
        ...
        for i in range(2 * int(eps * size)):
            ...
            eig_val, eig_vec = eigh(cov.cpu().numpy(), subset_by_index=(feature_size-1, feature_size-1), eigvals_only=False)
            eig_val = eig_val[0]
